    ...
    <v-card class="transparent card_mgleft mx-auto" outlined xs12 sm6 offset-sm3>
      ...
      <v-card-text>
        <v-row align="center" class="ma-5">
          <v-item-group v-model="window" class="shrink" mandatory tag="v-flex" v-if="items.folders != null">
            ...
            <v-window v-model="window" class="elevation-0" vertical v-if="items.folders != null">
              <v-window-item v-for="(folder, index) in items.folders" :key="index">
                <v-item-group>
                  <v-container>
                    <v-row>
                      <v-col v-for="(item, i) in folder.groups" :key="i" cols="12" md="4">
                        <v-item v-slot:default="{}">
                          <!-- <v-card :color="active ? 'primary' : ''" class="d-flex align-center" dark height="200" @click="toggle">
                        <v-scroll-y-transition>
                          <div v-if="active" class="display-3 flex-grow-1 text-center">{{ item.class }}</div>
                        </v-scroll-y-transition>
                          ...
                          <v-hover>
                            <v-card slot-scope="{ hover }" class="mx-auto" color="grey lighten-4">
                                ...
                                <v-expand-transition>
                                  ...
                                  >
                                    <v-card-text class="ma-0">
                                      ...
                                      <v-card-text class="ma-0">
                                        <div class="text-left pb-2" style="opacity: 1;">
                                          <v-icon small class="mb-1 mr-2" color="orange lighten-1">mdi-chef-hat</v-icon>Rajulas Kitchen
                                        </div>
                                        <p class="text-left pb-2">
                                          <v-icon small class="mb-1 mr-2" color="orange lighten-1">mdi-chef-hat</v-icon>102 people liked
                                        </p>
                                        <v-btn absolute color="orange" class="white--text" fab x-small right top style="top:10px; right: 5%">
                                          <v-icon color="white">mdi-heart</v-icon>
                                        </v-btn>
                                        <v-btn absolute color="orange" class="white--text" fab x-small right top style="top:10px; right: 15%">
                              ...
                              <v-btn absolute color="orange" class="white--text" fab small right bottom style="bottom:10px">
